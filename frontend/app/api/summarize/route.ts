...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) {
      console.log('Claude API key not configured, using fallback demo summary')
      // Fallback to demo summary if Claude API is not configured
    ...
    // Fallback to demo summary if Claude API fails
    const summary = generateDemoSummary()
    return NextResponse.json(summary)
  }
}

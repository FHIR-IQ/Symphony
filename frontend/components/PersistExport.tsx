      ...
      {!data.generatedSummary ? (
        ...
        <>
          ...
          <div className="border rounded-lg p-4 bg-gray-50">
            <h3 className="font-medium mb-2">HAPI FHIR Server Access</h3>
            <p className="text-sm text-gray-600 mb-3">Access the hosted HAPI FHIR server to view all resources and test data.</p>

            <div className="space-y-2">
              <a
                href={process.env.NEXT_PUBLIC_HAPI_WEB_URL || "https://hapi.fhir.org"}
                target="_blank"
                rel="noopener noreferrer"
              ...
              <a
                href={`${process.env.NEXT_PUBLIC_HAPI_BASE_URL || "https://hapi.fhir.org/baseR4"}/Composition`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors ml-2"
              >
                <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              ...
              {data.patientReference && (
                <div className="mt-3 p-3 bg-blue-50 rounded">
                  <div className="text-sm text-blue-800">
                    <div><strong>Patient Reference:</strong> {data.patientReference}</div>
                    <div><strong>FHIR Base URL:</strong> {process.env.NEXT_PUBLIC_HAPI_BASE_URL || "https://hapi.fhir.org/baseR4"}</div>
                  </div>
                  <a
                    href={`${process.env.NEXT_PUBLIC_HAPI_BASE_URL || "https://hapi.fhir.org/baseR4"}/${data.patientReference}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center mt-2 text-blue-600 hover:text-blue-800 text-sm underline"
                  >
